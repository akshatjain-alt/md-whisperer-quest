...
const schema = z.object({
  ...
  category: z.enum(['Cereal', 'Pulse', 'Vegetable', 'Fruit', 'Fiber', 'Oilseed']),
...
const CATEGORIES = ['Cereal', 'Pulse', 'Vegetable', 'Fruit', 'Fiber', 'Oilseed'] as const;
...
const columns: Column<Crop>[] = [
  ...
  { key: 'category', label: 'Category', render: (c) => <Badge variant="outline" className="text-xs">{c.category}</Badge> },
  { key: 'growing_season', label: 'Season', render: (c) => <Badge variant="secondary" className="text-xs">{c.growing_season}</Badge> },
];
...
export default function CropsPage() {
  ...
  const onSubmit = (data: FormData) => {
    if (editing) {
      updateCrop(editing.id, data);
      toast({ title: 'Crop updated successfully' });
    } else {
      addCrop(data as Omit<Crop, 'id' | 'created_at'>);
  ...
  return (
    <div className="animate-fade-in">
          ...
          <div className="bg-card rounded-xl border border-border p-5">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                ...
                <Input {...register('name')} placeholder="Enter crop name" />
                {errors.name && <p className="text-xs text-destructive mt-1">{errors.name.message}</p>}
              </div>
              <div>
                <Label>Local Name (Hindi)</Label>
                <Input {...register('local_name')} placeholder="स्थानीय नाम" />
              </div>
              <div>
                <Label>Scientific Name</Label>
                <Input {...register('scientific_name')} placeholder="Latin name" />
              </div>
              <div>
                <Label>Category *</Label>
                <Select defaultValue={editing?.category || 'Cereal'} onValueChange={(v) => setValue('category', v as any)}>
              ...
              <div>
                ...
                <Select defaultValue={editing?.growing_season || 'Kharif'} onValueChange={(v) => setValue('growing_season', v as any)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Textarea {...register('description')} placeholder="Describe the crop..." rows={3} />
              </div>
              <div className="flex gap-2 pt-2">
                <Button type="button" variant="outline" onClick={() => setShowForm(false)} className="flex-1">Cancel</Button>
                <Button type="submit" className="flex-1 bg-primary text-primary-foreground hover:bg-primary-light">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { deleteCrop(deleteId!); setDeleteId(null); toast({ title: 'Crop deleted' }); }}
      />
    </div>
  );
}
